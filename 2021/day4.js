...
function part2([drawnNumbers, boards]) {
    ...
    for (let dn = 0; dn < drawnNumbers.length; dn++) {
        ...
        if (dn >= 5) {
            ...
            for (const board of boards) {
                ...
                if (winningBoard) {
                    winningBoards.push([...winningBoard.map((wb) => [...wb])]);
                }
            }
        }
    }

    console.log(winningBoards.filter((wb) => {
        let stop = false;
        let stopCalled = 0;

        for (const col of wb) {
            stop = col.filter((e) => e < 0).length === 5;
    ...
    unmatchedSum = lastWinningBoard.flat().filter((e) => e !== -1).reduce((p, n) => p + n, 0);

    console.log(unmatchedSum);
